...
public class GenUtils {
    ...
    static {
        ...
        queryType2Suffix.put("NE", "NotEqual");
        queryType2Suffix.put("GT", "GreatThan");
        queryType2Suffix.put("GTE", "GreatThanOrEqual");
        queryType2Suffix.put("LT", "LessThan");
    ...
    static {
        ...
        queryType2Method.put("NE", "ne");
        queryType2Method.put("GT", "gt");
        queryType2Method.put("GTE", "ge");
        queryType2Method.put("LT", "lt");
    ...
    public static void initColumnField(GenTableColumn column, GenTable table) {
        ...
        if (StringUtils.isEmpty(column.getColumnComment())) {
            column.setColumnComment(column.getColumnName().toUpperCase());
        }
        if (arraysContains(GenConstants.COLUMNTYPE_STR, dataType) || arraysContains(GenConstants.COLUMNTYPE_TEXT, dataType)) {
        ...
        } else if (StrUtil.equals(dataType, "datetime")) {
            column.setJavaType("LocalDateTime");
            column.setHtmlType(GenConstants.HTML_DATETIME);
        } else if (StrUtil.equals(dataType, "date")) {
            column.setJavaType("LocalDate");
            column.setHtmlType("date");
        } else if (StrUtil.equals(dataType, "time")) {
            column.setJavaType("LocalTime");
            column.setHtmlType("time");
        } else if (StrUtil.equals(dataType, "int")) {
            ...
            if (str == null) {
                if (arraysContains(GenConstants.COLUMNTYPE_INT, dataType)) {
                    column.setJavaType(GenConstants.TYPE_INTEGER);
            ...
            } else if (str.length == 2 && Integer.parseInt(str[1]) > 0) {
                column.setJavaType(GenConstants.TYPE_BIGDECIMAL);
            }
            // 如果是整形
        ...
        // 图片字段设置图片上传控件
        else if (StringUtils.endsWithIgnoreCase(columnName, "image") || StringUtils.endsWithIgnoreCase(columnName, "images") || StringUtils.endsWithIgnoreCase(columnName, "photos")) {
    ...
    public static String getDbType(String columnType) {
        ...
        } else if (columnType.toLowerCase().endsWith(" unsigned")) {
            return columnType.substring(0, columnType.length() - 9).toLowerCase();
        } else {
            return columnType;
        }
    }
